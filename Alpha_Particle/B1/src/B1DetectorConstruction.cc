...
const G4int nEntries = 5;
  ...
  G4double l_lAr[nEntries] = {3448*m,  4082*m,  6329*m,  9174*m, 12346*m };
    ...
    G4ThreeVector pos5 = G4ThreeVector(0, -0.75*cm, 0.5*(-0.5*H_b + 0.5*espessura_maior + dif_h - esp));
   /* G4ThreeVector pos6 = G4ThreeVector(0, 3.0*cm - 0.5*esp_alpha, 0.5*(-0.5*H_b + 0.5*espessura_maior + dif_h - esp));*/
    ...
    G4ThreeVector pos11 = G4ThreeVector(0, -0.75*cm + 0.25*espeAra, 0.5*(-0.5*H_b + 0.5*espessura_maior + dif_h - esp));
    G4ThreeVector pos12 = G4ThreeVector(0, -0.75*cm - 0.25*espeAra, 0.5*(-0.5*H_b + 0.5*espessura_maior + dif_h - esp));
    ...
    G4ThreeVector trans7 = G4ThreeVector(0, -0.75*cm, -0.5*(h_b + espessura_menor)+0.5*esp + a_menor);
    ...
    G4ThreeVector trans11 = G4ThreeVector(0, -0.75*cm + 0.25*espeAra, -0.5*(h_b + espessura_menor)+0.5*esp + a_menor);
    G4ThreeVector trans12 = G4ThreeVector(0, -0.75*cm - 0.25*espeAra, -0.5*(h_b + espessura_menor)+0.5*esp + a_menor);
    ...
    G4RotationMatrix* rotacao = new G4RotationMatrix();
    ...
    G4Tubs* alumino_alpha = new G4Tubs("alumino_alpha", 0, 0.5*diam_buraco - 0.001*cm, 0.5*esp_aluminio, 0.0*deg, 360*deg);
    ...
    G4VPhysicalVolume* phys_Tampa = new G4PVPlacement(rotacao,                       //no rotation
                                                        pos10,                 //at position
    ...
    G4SubtractionSolid* sub6 = new G4SubtractionSolid("sub6", sub5, alumino_alpha, rotacao, trans8);
    G4SubtractionSolid* sub7 = new G4SubtractionSolid("sub7", sub6, tampa, rotacao, trans10);
    ...
    prop_argonio->AddConstProperty("SCINTILLATIONYIELD", 3621.0/MeV);
    ...
    prop_argonio->AddConstProperty("YIELDRATIO", 0.25); // As here
